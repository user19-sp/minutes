...
The database URL is read from application settings, not from alembic.ini, so
migrations always target the same database the app does and there is no second
place to keep a credential in sync. alembic.ini therefore carries no URL.
"""
...
from backend.app import models  # noqa: E402,F401  (registers every mapper)
